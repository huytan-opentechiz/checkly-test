...
//     const difference = Math.abs(pageHeight - 8350);
...
//     }
//   }
//   await page.goto("https://www.glamira.ch/sylvie-ring-teselya-a.html?alloy=yellow_white-585&stone1=lab-grown-diamond&bypass_varnish=1", { waitUntil: 'domcontentloaded' });
//   await compareHeight()
//   await page.waitForTimeout(300)
//   await page.goto("https://www.glamira.ch");
//   await page.waitForLoadState('load')
//   await page.locator("//div[@class='top-menu']//a[@title='Schmuck']").hover()
//   await page.waitForLoadState('load')
//   await page.waitForTimeout(200)
//   await page.locator("(//nav[@id='menu_mega']//a[@title='Ringe'])[1]").hover()
//   await page.waitForTimeout(200)
//   await page.locator("(//nav[@id='menu_mega']//a[@title='Initialen & Namensringe'])[1]").click()
//   await page.evaluate(() => window.scrollTo(0, 500))
//   await page.locator("//div[@class='item label filter-initials']//a[@data-url-key='initials-a']").click()
//   await page.waitForLoadState('load')
//   await page.evaluate(() => window.scrollTo(0, 800))
//   const priceInListing = await page.locator("//a[@title='SYLVIE Ring Teselya - A']//following::span[@class='price'][1]").textContent()
//   await page.locator("(//a[@title='SYLVIE Ring Teselya - A']//span)[1]").click()
//   await page.waitForLoadState('load')
...
//   const hreflang = page.locator("//link[@hreflang='de-CH']")
...
//   await page.waitForLoadState('load')
//   await username.fill("cuong+ch@onlinebizsoft.com")
//   await page.locator("#checkout-login-password").fill("Duccuong7.")
//   await page.waitForTimeout(100);
//   await page
//     .locator(
...
test("Check the flow of store activities", async ({ page }) => {
  ...
  const userName = "cuong+ch@onlinebizsoft.com"
  const password = "Duccuong7."
  const titleRemoveSuccess = "Einkaufswagen ist leer" // thay text theo ngôn ngữ store ( giỏ hàng đang trống)
  const URL_STORE = "https://www.glamira.ch" // thay url store \

  const URL_PRODUCT_PAGE = "https://www.glamira.ch/diamantringe/"

  const URL_CHECK_OUT_CART = "https://www.glamira.ch/checkout/cart/"
  const URL_CHECK_OUT_LOGIN_PAGE =
    "https://www.glamira.ch/checkout/#login-address"
  ...
  const BUTTON_ALL_WOMEN_JEWELLERY = await page.locator(
    'a.pagebuilder-button-link[href="https://www.glamira.ch/damen/"]',
  ) // thay text theo ngôn ngữ store
  const BUTTON_VIEW_All_RINGS = await page.locator(
    "//span[text()='Alle Ringe anzeigen']",
  ) // thay text theo ngôn ngữ store
  ...
  const PAYMENT_WITH_TWINT = await page.locator(
    "//label[@for='stripe_payments_twint']",
  )
  const PAYMENT_WITH_PAYPAL = await page.locator(
    "//label[@for='stripe_payments_paypal']",
  )
  const PAYMENT_WITH_TRANSFER = await page.locator("//label[@for='checkmo']")
  ...
  async function ClickProcessAndCheckPayment() {
    ...
    await expect(PAYMENT_WITH_TWINT).toBeVisible()
