...
//     const diff1 = Math.abs(pageHeight - 12600);
//     console.log('difference compare to expect == ' + diff1 + ' pixels');
//     await expect(diff1).toBeLessThan(500);
...
//    await page.goto("https://www.glamira.hk/en/glamira-pendant-elsie.html?alloy=red_white-585&stone1=blackdiamond&bypass_varnish=1", { waitUntil: 'domcontentloaded' });
...
//     await cookies.click()
//   }
//   await page.goto("https://www.glamira.hk/en", { waitUntil: 'domcontentloaded' });
//   await page.evaluate(() => window.scrollTo(0, 2000))
//   await page.waitForTimeout(1000);
//   await page.evaluate(() => window.scrollTo(5000, 0))
...
//   // const hreflang = page.locator("(//link[@hreflang='en-HK'])")
//   // const linkCanoical = await canoical.getAttribute("href")
//   // const linkHref = await hreflang.getAttribute("href")
//   // await expect(linkCanoical).toEqual("https://www.glamira.hk/en/glamira-bracelet-tanel.html?alloy=white_red-375&stone1=diamond-Brillant")
...
//   await username.fill("ly+hk/en@onlinebizsoft.com")
...
test("Check the flow of store activities", async ({ page }) => {
  ...
  const userName = "ly+hk/en@onlinebizsoft.com"
  const password = "Linh@123"
  const titleRemoveSuccess = "Shopping Bag is Empty" // thay text theo ngôn ngữ store ( giỏ hàng đang trống)
  const URL_STORE = "https://www.glamira.hk/en" // thay url store
  const URL_PRODUCT_PAGE = "https://www.glamira.hk/en/diamond-rings/"
  const URL_CHECK_OUT_CART = "https://www.glamira.hk/en/checkout/cart/"
  const URL_CHECK_OUT_LOGIN_PAGE =
    "https://www.glamira.hk/en/checkout/#login-address"
  ...
  const BUTTON_ALL_WOMEN_JEWELLERY = await page.locator(
    'a.pagebuilder-button-link[href="https://www.glamira.hk/en/women/"]',
  ) // thay text theo ngôn ngữ store
