...
//     const diff1 = Math.abs(pageHeight - 13500);
...
//   await page.goto("https://www.glamira.az/glamira-pendant-elsie.html?alloy=red_white-585&stone1=blackdiamond&bypass_varnish=1", { waitUntil: 'domcontentloaded' });
...
//   await page.goto("https://www.glamira.az", { waitUntil: 'domcontentloaded' });
//   await page.evaluate(() => window.scrollTo(0, 2000))
//   await page.waitForTimeout(1000);
//   await page.evaluate(() => window.scrollTo(5000, 0))
//   await page.waitForTimeout(3000);
...
//   await page.waitForSelector("//div[@data-content-type='button-item']//span[text()='Kolyeler']")
//   await page.click("//div[@data-content-type='button-item']//span[text()='Kolyeler']")
...
//   await page.click("//div[@id='content_filter_more_selected']//li[@class='item label  filter-diamonds']//a[text()='Pırlanta']")
//   await page.waitForTimeout(100);
//   await page.click("//div[@id='content_filter_more_selected']//label[text()='Siyah Pırlanta']")
//   await page.waitForLoadState('load');
...
//   await page.goto("https://www.glamira.az/glamira-pendant-elsie.html?alloy=red_white-585&stone1=blackdiamond", { waitUntil: 'domcontentloaded' });
//   await goToPageCompareHeight()
//   const canoical = page.locator("//link[@rel='canonical']")
//   const hreflang = page.locator("(//link[@hreflang='tr-AZ'])")
//   const linkCanoical = await canoical.getAttribute("href")
//   const linkHref = await hreflang.getAttribute("href")
//   await expect(linkCanoical).toEqual("https://www.glamira.az/glamira-pendant-elsie.html?alloy=red_white-585&stone1=blackdiamond")
...
//   await page.locator("//div[@id='stone']//span[text()='Pırlanta']").click()
//   await page.waitForTimeout(200)
...
//   await username.fill("ly+az@onlinebizsoft.com")
...
test("Check the flow of store activities", async ({ page, playwright }) => {
  test.setTimeout(220000)
  await page.setViewportSize({ width: 400, height: 768 })
  //   // khai báo các locator
  const userName = "ly+az@onlinebizsoft.com"
  const password = "Linh@123"
  const titleRemoveSuccess = "ALIŞVERİŞ SEPETİNİZ BOŞ" // thay text theo ngôn ngữ store ( giỏ hàng đang trống)
  const URL_STORE = "https://www.glamira.az" // thay url store
  const URL_PRODUCT_PAGE = "https://www.glamira.az/pirlanta-yuzuk/"
  const URL_CHECK_OUT_CART = "https://www.glamira.az/checkout/cart/"
  const URL_CHECK_OUT_LOGIN_PAGE =
    "https://www.glamira.az/checkout/#login-address"
  ...
  const BUTTON_ALL_WOMEN_JEWELLERY = await page.locator(
    'a.pagebuilder-button-link[href="https://www.glamira.az/kadin/"]',
  ) // thay text theo ngôn ngữ store
  const BUTTON_VIEW_All_RINGS = await page.locator(
    "//span[text()='Tüm Yüzükleri Görüntüle']",
  ) // thay text theo ngôn ngữ store
  ...
  async function checkPriceAfterClickLogoAndRhodiumOption() {
    ...
    const specialPriceProductPage = await PRODUCT_PAGE_SPECIAL_PRICE_MOBILE.textContent();
