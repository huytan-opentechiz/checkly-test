...
//   await page.goto("https://www.glamira.md/glamira-bracelet-tanel.html?alloy=white_red-375&stone1=diamond-Brillant&no_cache=1", { waitUntil: 'domcontentloaded' });
//   await compareHeight()
//   // await page.goto("https://www.glamira.md/glamira-bracelet-tanel.html?alloy=white_red-375&stone1=diamond-Brillant&bypass_varnish=1", { waitUntil: 'domcontentloaded' });
//   // await compareHeight()
//   await page.waitForTimeout(300)
//   await page.goto("https://www.glamira.md");
//   await page.waitForLoadState('load')
//   await page.locator("//div[@class='top-menu']//a[@title='Bijuterii']").click()
//   await page.waitForLoadState('load')
//   await page.waitForTimeout(200)
//   await page.locator("(//nav[@id='menu_mega']//a[@title='Brățări'])[1]").click()
//   await page.waitForLoadState('load')
//   await page.evaluate(() => window.scrollTo(0, 800))
//   await page.locator("(//li[@class='item label  filter-diamonds'])[1]").click()
//   await page.waitForTimeout(200)
//   await page.locator("(//li[@class='item label  filter-diamonds'])[1]//a[@data-filter-value='blackdiamond']").click()
//   await page.waitForLoadState('load')
//   const priceInListing = await page.locator("//a[contains(@title,'Diamant Negru Dama Brățară Tressa')]//following::span[@class='price'][1]").textContent()
//   await page.locator("(//a[contains(@title,'Tressa')]//span)[1]").click()
//   await page.waitForLoadState('load')
...
//   // Add to cart
//   await page.waitForTimeout(200);
//   await page.locator("//div[@id='stone']//span[text()='Diamant cultivat în laborator']").click()
...
//   await page.waitForTimeout(500);
//   await username.fill("cuong+md@onlinebizsoft.com")
...
//   await billing.click()
//   // await page.waitForTimeout(300);
//   // const stripePayment = await page.locator(
//   //   "//div[@class='payment-method-title field choice']//label[@for='stripe_payments']"
//   // )
...
test("Check the flow of store activities", async({ page }) => {
  ...
  const userName = "cuong+md@onlinebizsoft.com";
  const password = "Duccuong7." ;
  const titleRemoveSuccess = "Coșul de Cumpărături este Gol" // thay text theo ngôn ngữ store ( giỏ hàng đang trống)
  const URL_STORE = "https://www.glamira.md"; // thay url store 
  const URL_PRODUCT_PAGE = "https://www.glamira.md/inele-diamant/";
  const URL_CHECK_OUT_CART = "https://www.glamira.md/checkout/cart/"
  const URL_CHECK_OUT_LOGIN_PAGE = "https://www.glamira.md/checkout/#login-address";
  ...
  const BUTTON_ACCEPT_COOKIE =await  page.locator("//button[text()='Accept Toate Cookie-Urile']"); // thay text theo ngôn ngữ store
  const BUTTON_JEWELLERY =await  page.locator("//div[@class='top-menu']//li[1]");
  const BUTTON_ALL_WOMEN_JEWELLERY =await  page.locator('a.pagebuilder-button-link[href="https://www.glamira.md/femei/"]'); // thay text theo ngôn ngữ store
  const BUTTON_VIEW_All_RINGS =await  page.locator("//span[text()='Vizualizează toate inelele']") // thay text theo ngôn ngữ store
  const TEXT_PRICE_PRODUCTION_LISTING_PAGE =await  page.locator("//span[@id='product-price-115482']//span[@class='price']") ;
  const PRODUCTION_LISTING_PAGE =await  page.locator("//h2[text()=' Inel Soderman']"); // thay text theo ngôn ngữ store hãy lấy đúng sản phẩm tôi so sánh giá ở trên
  ...
  const PAYMENT_WITH_CREDIT_CARD = await page.locator("#stripe-payment-element");
      ...
      anchors
        .filter((a) => {
          ...
          return (
            ...
            !href.startsWith(
              `https://www.glamira.md/bratara/bratari_din_piele`,
            ) &&
            !href.startsWith(
              `https://www.glamira.md/your-story-lp/`,
            ) &&
