...
//     const difference = Math.abs(pageHeight - 9200);
...
//     }
//   }
//   await page.goto("https://www.glamira.az/en/glamira-bracelet-tanel.html?alloy=white_red-375&stone1=diamond-Brillant&no_cache=1", { waitUntil: 'domcontentloaded' });
//   await compareHeight()
//   // await page.goto("https://www.glamira.az/en/glamira-bracelet-tanel.html?alloy=white_red-375&stone1=diamond-Brillant&bypass_varnish=1", { waitUntil: 'domcontentloaded' });
//   // await compareHeight()
//   await page.waitForTimeout(300)
//   await page.goto("https://www.glamira.az/en");
...
//   await page.waitForLoadState('load')
//   const priceInListing = await page.locator("//a[contains(@title,'Bracelet Tressa')]//following::span[@class='price'][1]").textContent()
//   await page.locator("(//a[contains(@title,'Bracelet Tressa')]//span)[1]").click()
//   await page.waitForLoadState('load')
...
//   const hreflang = page.locator("//link[@hreflang='en-AZ']")
...
//   await username.fill("cuong+az/en@onlinebizsoft.com")
...
test("Check the flow of store activities", async ({ page }) => {
  ...
  const userName = "cuong+az/en@onlinebizsoft.com"
  const password = "Duccuong7."
  const titleRemoveSuccess = "Shopping Bag is Empty" // thay text theo ngôn ngữ store ( giỏ hàng đang trống)
  const URL_STORE = "https://www.glamira.az/en/" // thay url store \

  const URL_PRODUCT_PAGE = "https://www.glamira.az/en/diamond-rings/"

  const URL_CHECK_OUT_CART = "https://www.glamira.az/en/checkout/cart/"
  const URL_CHECK_OUT_LOGIN_PAGE =
    "https://www.glamira.az/en/checkout/#login-address"
  ...
  const BUTTON_ALL_WOMEN_JEWELLERY = await page.locator(
    'a.pagebuilder-button-link[href="https://www.glamira.az/en/women/"]',
  ) // thay text theo ngôn ngữ store
