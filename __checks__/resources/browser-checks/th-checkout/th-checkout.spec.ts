...
//     const difference = Math.abs(pageHeight - 9000);
...
//     }
//   }
//   await page.goto("https://www.glamira.co.th/glamira-bracelet-tanel.html?alloy=white_red-375&stone1=diamond-Brillant&bypass_varnish=1", { waitUntil: 'domcontentloaded' });
//   await compareHeight()
//   await page.waitForTimeout(300)
//   await page.goto("https://www.glamira.co.th");
//   await page.waitForLoadState('load')
//   await page.locator("//div[@class='top-menu']//a[@title='เครื่องประดับ']").click()
//   await page.waitForLoadState('load')
//   await page.waitForTimeout(100)
//   await page.locator("(//nav[@id='menu_mega']//a[@title='สร้อยข้อมือ'])[1]").click()
//   await page.waitForLoadState('load')
...
//   const priceInListing = await page.locator("//a[@title='เพชรสีดำ สร้อยข้อมือ Tressa']//following::span[@class='price'][1]").textContent()
//   await page.locator("(//a[@title='เพชรสีดำ สร้อยข้อมือ Tressa']//span)[1]").click()
//   await page.waitForLoadState('load')
...
//   const hreflang = page.locator("(//link[@hreflang='th-TH'])")
...
//   await username.fill("cuong+th@onlinebizsoft.com")
...
test("Check the flow of store activities", async({ page }) => {
  ...
  const userName = "cuong+th@onlinebizsoft.com";
  const password = "Duccuong7." ;
  const titleRemoveSuccess = "ถุงช้อปปิ้งว่างเปล่า" // thay text theo ngôn ngữ store ( giỏ hàng đang trống)
  const URL_STORE = "https://www.glamira.co.th/"; // thay url store 
  const URL_PRODUCT_PAGE = "https://www.glamira.co.th/diamond-rings/";
  const URL_CHECK_OUT_CART = "https://www.glamira.co.th/checkout/cart/"
  const URL_CHECK_OUT_LOGIN_PAGE = "https://www.glamira.co.th/checkout/#login-address";
  ...
  const BUTTON_ACCEPT_COOKIE =await  page.locator("//button[text()='ยอมรับคุกกี้ทั้งหมด']"); // thay text theo ngôn ngữ store
  const BUTTON_JEWELLERY =await  page.locator("//div[@class='top-menu']//li[1]");
  const BUTTON_ALL_WOMEN_JEWELLERY =await  page.locator('a.pagebuilder-button-link[href="https://www.glamira.co.th/women/"]'); // thay text theo ngôn ngữ store
  const BUTTON_VIEW_All_RINGS =await  page.locator("//span[text()='ดูแหวนทั้งหมด']") // thay text theo ngôn ngữ store
  const TEXT_PRICE_PRODUCTION_LISTING_PAGE =await  page.locator("//span[@id='product-price-115482']//span[@class='price']") ;
  const PRODUCTION_LISTING_PAGE =await  page.locator("//h2[text()=' แหวน Soderman']"); // thay text theo ngôn ngữ store hãy lấy đúng sản phẩm tôi so sánh giá ở trên
 ...
 async function compareHeightPage() {
    try {
      ...
      console.log('Chiều cao của trang là: ' + pageHeight + ' pixels');
      const difference = Math.abs(pageHeight - 7500);
      console.log('Pixel sai lệch so với mong đợi = ' + difference);
      await expect(difference).toBeLessThan(500);
    } catch (error) {
