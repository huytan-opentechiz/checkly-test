...
//     }
//   }
//   // await goPageAndAddCart("https://www.glamira.com.mx/glamira-bracelet-tanel.html?alloy=white_red-375&stone1=diamond-Brillant&no_cache=1")
//   await page.goto("https://www.glamira.com.mx/glamira-bracelet-tanel.html?alloy=white_red-585&stone1=diamond-Brillant&bypass_varnish=1", { waitUntil: 'domcontentloaded' });
//   await compareHeight(9350)
//   await page.goto("https://www.glamira.com.mx/glamira-bracelet-tanel.html?alloy=white_red-585&stone1=diamond-Brillant&utm_source=monitoring", { waitUntil: 'domcontentloaded' });
//   // Check hreflang
...
//   const hreflang = page.locator("(//link[@hreflang='es-MX'])")
//   const linkCanoical = await canoical.getAttribute("href")
//   const linkHref = await hreflang.getAttribute("href")
//   await expect(linkCanoical).toEqual(baseUrl.replace("&utm_source=monitoring",""))
//   await expect(linkCanoical).toEqual(linkHref)

//   // Go homepage
//   await page.waitForTimeout(300)
//   await page.goto("https://www.glamira.com.mx");
//   await page.waitForLoadState('load')
//   await page.waitForTimeout(300)
//   await page.locator("//div[@class='top-menu']//a[@title='Joyería']").hover();
//   await page.waitForTimeout(200)
//   await page.locator("(//nav[@id='menu_mega']//a[@title='Collares'])[1]").click()
//   await page.waitForLoadState('load')
//   await page.evaluate(() => window.scrollTo(0, 800))
//   await page.waitForTimeout(200)
//   await page.locator("(//li[@class='item label  filter-diamonds'])[8]").click()
//   await page.waitForTimeout(200)
//   await page.locator("(//li[@class='item label  filter-diamonds'])[8]//a[@data-url-key='turquoise']").click()
//   await page.waitForLoadState('load')
//   const priceInListing = await page.locator("//a[contains(@title,'Barrejat')]//following::span[@class='price'][1]").textContent()
//   await page.locator("(//a[contains(@title,'Barrejat')]//span)[1]").click()
//   await page.waitForLoadState('load')
...
//   await page.locator("(//div[@id='stone']//span[@class='option_label_title'])[2]").click()
//   await page.locator("//div[@id='stone']//img[@alt='Diamante']").click()
...
//   await page.waitForTimeout(1000);
//   await username.fill("cuong+mx@onlinebizsoft.com")
...
test("Check the flow of store activities", async ({ page }) => {
  ...
  const userName = "cuong+mx@onlinebizsoft.com"
  const password = "Duccuong7."
  const titleRemoveSuccess = "El carrito de compras está vacío" // thay text theo ngôn ngữ store ( giỏ hàng đang trống)
  const URL_STORE = "https://www.glamira.com.mx" // thay url store
  const URL_PRODUCT_PAGE = "https://www.glamira.com.mx/anillos-de-diamantes/"
  const URL_CHECK_OUT_CART = "https://www.glamira.com.mx/checkout/cart/"
  const URL_CHECK_OUT_LOGIN_PAGE =
    "https://www.glamira.com.mx/checkout/#login-address"
  const URL_CHECK_OUT_SHIPPING = "https://www.glamira.mx/checkout/#shipping"
  ...
  const PAYMENT_WITH_CREDIT_CARD = await page.locator(
    "//div[@class='payment-method stripe-payments']//div[@class='payment-method-title field choice'][1]",
  )
  ...
  async function scrollToBottom(scrollAmount = 300, interval = 1000) {
    const distanceToScroll = async () => {
      return await page.evaluate(
        () => document.body.scrollHeight - window.innerHeight - window.scrollY,
      )
    }

    while ((await distanceToScroll()) > 0) {
      // Cuộn xuống
      await page.evaluate((amount) => window.scrollBy(0, amount), scrollAmount)

      // Chờ một khoảng thời gian trước khi cuộn tiếp
  ...
  // async function compareHeightPage(expectedHeight) {
  ...
  //   await page.evaluate(() => window.scroll(document.body.scrollHeight, 0))
  ...
  //   })
  //   console.log("Chiều cao của trang là: " + pageHeight + " pixels")
  //   const difference = Math.abs(pageHeight - expectedHeight)
  //   console.log("Pixel sai lệch so với mong đợi = " + difference)
      ...
      anchors
        .filter((a) => {
          ...
          return (
            ...
            !href.startsWith(`https://www.glamira.com.mx/4`) &&
            !href.startsWith(
              `https://www.glamira.com.mx/anillos-pareja/925-silver/`,
            ) &&
  ...
  async function clickButtonAddToShoppingBagAndCheckAddSuccess() {
    ...
    await page.waitForLoadState("load")
    await page.waitForTimeout(5000)
    await BUTTON_ADD_TO_SHOPPING.waitFor({ state: "visible" })
    await BUTTON_ADD_TO_SHOPPING.scrollIntoViewIfNeeded()
