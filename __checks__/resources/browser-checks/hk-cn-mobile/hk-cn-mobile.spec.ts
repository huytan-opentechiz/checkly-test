...
//     await page.evaluate(() => window.scrollTo(9000, 0))
//     await page.waitForTimeout(2500);
...
//     const diff1 = Math.abs(pageHeight - 11400);
//     console.log('difference compare to expect == ' + diff1 + ' pixels');
//     await expect(diff1).toBeLessThan(600);
...
//    await page.goto("https://www.glamira.hk/cn/glamira-pendant-elsie.html?alloy=red_white-585&stone1=blackdiamond&bypass_varnish=1", { waitUntil: 'domcontentloaded' });
...
//     await cookies.click()
//   }
//   await page.goto("https://www.glamira.hk/cn", { waitUntil: 'domcontentloaded' });
//   await page.evaluate(() => window.scrollTo(0, 2000))
//   await page.waitForTimeout(1000);
//   await page.evaluate(() => window.scrollTo(5000, 0))
//   await page.waitForTimeout(2500);
...
//   await page.waitForSelector("(//div[@class='pagebuilder-column']//a[@class='pagebuilder-button-link'])[2]")
//   await page.click("(//div[@class='pagebuilder-column']//a[@class='pagebuilder-button-link'])[2]")
//   await page.waitForLoadState('load');
//   await page.evaluate(() => window.scrollTo(0, 500))
//   await page.click("//button[@class='btn__mb__filter']")
//   await page.waitForLoadState('load');
//   await page.waitForTimeout(200);
//   await page.click("(//div[@id='content_filter_more_selected']//li[@class='item label  filter-diamonds'])[1]")
//   await page.waitForTimeout(200);
//   await page.click("//div[@id='content_filter_more_selected']//img[@src='https://cdn-media.glamira.com/media/diamonds/stone_redesign/blackdiamond.png']")
//   await page.waitForLoadState('load');
//   await page.click("//div[@id='content_filter_more_selected']//div[@class='action action-apply-filter']")
//   await page.reload();
//   await page.waitForLoadState('load');
//   const price = await page.locator("(//h2[@class='product-item-details product-name'])[1]//following::span[@class='price'][1]").textContent()
...
//   // const hreflang = page.locator("//link[@hreflang='zh-Hans-HK']")
//   // const linkCanoical = await canoical.getAttribute("href")
//   // const linkHref = await hreflang.getAttribute("href")
//   // await expect(linkCanoical).toEqual("https://www.glamira.hk/cn/glamira-bracelet-tanel.html?alloy=white_red-375&stone1=diamond-Brillant")
//   // await expect(linkCanoical).toEqual(linkHref)
//   await page.goto("https://www.glamira.hk/cn/glamira-pendant-elsie.html?alloy=red_white-585&stone1=blackdiamond", { waitUntil: 'domcontentloaded' });
//   await goToPageCompareHeight()
//   await page.waitForLoadState('load');
//     // check price range
...
//   // add to cart
//   await page.locator("#stone > div.option_label.inactive.has-view-mode").click()
//   await page.waitForTimeout(1000);
//   await page.locator("//div[@id='stone']//span[text()='鑽石']").click()
...
//   await page.waitForTimeout(1000);
//   await username.fill("ly+hk/cn@onlinebizsoft.com")
...
test("Check the flow of store activities", async ({ page }) => {
  ...
  const userName = "ly+hk/cn@onlinebizsoft.com"
  const password = "Linh@123"
  const titleRemoveSuccess = "购物篮是空的" // thay text theo ngôn ngữ store ( giỏ hàng đang trống)
  const URL_STORE = "https://www.glamira.hk/cn" // thay url store
  const URL_PRODUCT_PAGE = "https://www.glamira.hk/cn/%E9%92%BB%E6%88%92/"
  const URL_CHECK_OUT_CART = "https://www.glamira.hk/cn/checkout/cart/"
  const URL_CHECK_OUT_LOGIN_PAGE =
    "https://www.glamira.hk/cn/checkout/#login-address"
  ...
  const PAYMENT_WITH_ALIPAY = await page.locator(
    "//div[@class='payment-method']//div[@class='payment-method-title field choice']//label[@for='adyen_alipay']",
  )
  const PAYMENT_WITH_PAYPAL = await page.locator(
    "//div[@class='payment-method']//div[@class='payment-method-title field choice']//label[@for='paypal_express']",
  )
  ...
  async function ClickProcessAndCheckPayment() {
    ...
    // await expect(PAYMENT_WITH_ALIPAY).toBeVisible();
